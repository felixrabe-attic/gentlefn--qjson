...
QByteArray Serializer::SerializerPrivate::serialize( const QVariant &v, int reserved )
{
  ...
  if ( ! v.isValid() ) { // invalid or null?
    ...
    else if (indentMode == QJson::IndentNoneCompact) {
      str = "[" + join( values, "," ) + "]";
    }
    else {
      str = "[ " + join( values, ", " ) + " ]";
    ...
    QByteArray kvDelim = (indentMode == QJson::IndentNoneCompact) ? ":" : " : ";
    ...
    while ( it.hasNext() ) {
      ...
      pairs << sanitizeString( it.key() ).toUtf8() + kvDelim + serializedValue;
    }
